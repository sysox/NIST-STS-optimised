...
nist_test_suite()
{
	if ( (testVector[0] == 1) || (testVector[TEST_FREQUENCY] == 1) ) 
		if(tp.fast) Frequency2(tp.n); else Frequency(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_BLOCK_FREQUENCY] == 1) ) 
		if(tp.fast) BlockFrequency2(tp.blockFrequencyBlockLength, tp.n); else BlockFrequency(tp.blockFrequencyBlockLength, tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_CUSUM] == 1) )
		if(tp.fast) CumulativeSums2(tp.n); else CumulativeSums(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_RUNS] == 1) )
		if(tp.fast) Runs2(tp.n); else Runs(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_LONGEST_RUN] == 1) )
		if(tp.fast) LongestRunOfOnes2(tp.n); else LongestRunOfOnes(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_RANK] == 1) )
		if(tp.fast) Rank2(tp.n); else Rank(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_FFT] == 1) )
		if(tp.fast) DiscreteFourierTransform2(tp.n); else DiscreteFourierTransform(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_NONPERIODIC] == 1) )
		if(tp.fast) NonOverlappingTemplateMatchings2(tp.nonOverlappingTemplateBlockLength, tp.n);
		else NonOverlappingTemplateMatchings(tp.nonOverlappingTemplateBlockLength, tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_OVERLAPPING] == 1) )
		if(tp.fast) OverlappingTemplateMatchings2(tp.overlappingTemplateBlockLength, tp.n);
		else OverlappingTemplateMatchings(tp.overlappingTemplateBlockLength, tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_UNIVERSAL] == 1) )
		if(tp.fast) Universal2(tp.n); else Universal(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_APEN] == 1) )
		if(tp.fast) ApproximateEntropy2(tp.approximateEntropyBlockLength, tp.n);
		else ApproximateEntropy(tp.approximateEntropyBlockLength, tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_RND_EXCURSION] == 1) )
		if(tp.fast) RandomExcursions2(tp.n); else RandomExcursions(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_RND_EXCURSION_VAR] == 1) )
		if(tp.fast) RandomExcursionsVariant2(tp.n); else RandomExcursionsVariant(tp.n);
	
	if ( (testVector[0] == 1) || (testVector[TEST_SERIAL] == 1) )
		if(tp.fast) Serial2(tp.serialBlockLength,tp.n); else Serial(tp.serialBlockLength,tp.n);
